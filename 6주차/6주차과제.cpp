...
float transmove_x = 0; //이동 기본 변수값
float rotationAngle = 0; //앵글 기본 변수값
float scalesize = 1; // 크기 기본 변수값
float scalesizeup = 0.01; //크기 증가 기본 변수값
...
void Update()
{
    while (!glfwWindowShouldClose(window))
    {
        ...
        transform.rotation = glm::mat3(
            glm::cos(glm::radians(rotationAngle)), -glm::sin(glm::radians(rotationAngle)), 0,
            glm::sin(glm::radians(rotationAngle)), glm::cos(glm::radians(rotationAngle)), 0,
            0, 0, 1
        );

        scalesize += scalesizeup;
        ...
        // 루프내에서 트랜스폼 변경을 수행할 경우 트랜스폼을 초기화한뒤에 트랜스폼을 변경을 수행해야한다.
